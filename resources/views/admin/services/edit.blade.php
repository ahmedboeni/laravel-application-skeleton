            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 dark:text-gray-400">
                        {{ __('app.edit_service_description', ['default' => 'تعديل معلومات الخدمة']) }}
                    </p>
                </div>
                ...
                <form action="{{ route('admin.services.update', $service) }}" method="POST">
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="carrier_id" 
                                ...
                                @foreach($carriers ?? [] as $carrier)
                                    <option value="{{ $carrier->id }}" {{ old('carrier_id', $service->carrier_id) == $carrier->id ? 'selected' : '' }}>
                                        {{ $carrier->name_ar }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="service_type" 
                                ...
                                @foreach(\App\Models\AppVariable::getByCategory('service_types') as $serviceType)
                                    <option value="{{ $serviceType->variable_key }}" 
                                            {{ old('service_type', $service->service_type) == $serviceType->variable_key ? 'selected' : '' }}
                    ...
                    <div class="mt-6">
                        ...
                        <textarea id="description" 
                                  ...
                                  placeholder="{{ __('app.service_description_placeholder', ['default' => 'وصف الخدمة...']) }}">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <!-- Hidden JSON Input -->
                            <input type="hidden" id="client_fields" name="client_fields" value="{{ old('client_fields', $service->client_fields ? json_encode($service->client_fields, JSON_UNESCAPED_UNICODE) : '') }}">
                        </div>
                        @error('client_fields')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea id="metadata_notes" 
                                          rows="2"
                                          class="form-textarea"
                                          placeholder="ملاحظات إضافية عن الخدمة...">{{ old('metadata_notes', isset($service->metadata['notes']) ? $service->metadata['notes'] : '') }}</textarea>
                            </div>
                            
                            <!-- Hidden JSON Input -->
                            <input type="hidden" id="metadata" name="metadata" value="{{ old('metadata', $service->metadata ? json_encode($service->metadata, JSON_UNESCAPED_UNICODE) : '') }}">
                        </div>
                        @error('metadata')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
...
// تحميل البيانات الموجودة عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    // تحميل client_fields الموجودة
    const existingClientFields = document.getElementById('client_fields').value;
    if (existingClientFields) {
        try {
            selectedClientFields = JSON.parse(existingClientFields);
            updateClientFieldsDisplay();
        } catch (e) {
            console.log('Error parsing existing client fields:', e);
        }
    }

    // تحميل metadata الموجودة
    const existingMetadata = document.getElementById('metadata').value;
    if (existingMetadata) {
        try {
            ...
            if (metadata.tags && Array.isArray(metadata.tags)) {
                metadata.tags.forEach(tagKey => {
                    const tagElement = document.querySelector(`[data-tag="${tagKey}"]`);
                    if (tagElement) {
                        const tagLabel = tagElement.textContent.trim();
                        const tagColor = tagElement.style.color || '#007bff';
                        selectedTags.push({ key: tagKey, label: tagLabel, color: tagColor });
            ...
            if (metadata.features && Array.isArray(metadata.features)) {
                metadata.features.forEach(featureKey => {
                    const featureElement = document.querySelector(`[data-feature="${featureKey}"]`);
                    if (featureElement) {
                        const featureLabel = featureElement.textContent.trim();
                        selectedFeatures.push({ key: featureKey, label: featureLabel });
                        featureElement.classList.remove('bg-white', 'text-gray-700', 'border-gray-300');
                        featureElement.classList.add('bg-primary-100', 'text-primary-800', 'border-primary-300');
                    }
                });
