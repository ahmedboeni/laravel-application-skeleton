...
@section('title', __('app.edit_service_option'))
...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900 dark:text-white mb-2">
                        {{ __('app.edit_service_option') }}
                    </h1>
                    <p class="text-gray-600 dark:text-gray-400">
                        {{ __('app.edit_service_option_description', ['default' => 'تعديل معلومات خيار الخدمة']) }}
                    </p>
                </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('admin.service-options.update', $serviceOption) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="service_id" 
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}" {{ old('service_id', $serviceOption->service_id) == $service->id ? 'selected' : '' }}>
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="provider_mapping_id" 
                                ...
                                @foreach($providerMappings ?? [] as $mapping)
                                    <option value="{{ $mapping->id }}" {{ old('provider_mapping_id', $serviceOption->provider_mapping_id) == $mapping->id ? 'selected' : '' }}>
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="number" 
                                   id="option_type" 
                                   name="option_type" 
                                   value="{{ old('option_type', $serviceOption->option_type) }}"
                                   min="1"
                                   step="1"
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="text" 
                                   id="option_name" 
                                   name="option_name" 
                                   value="{{ old('option_name', $serviceOption->option_name) }}"
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="client_field" 
                                ...
                                <option value="player_id" {{ old('client_field', $serviceOption->client_field) == 'player_id' ? 'selected' : '' }}>رقم اللاعب</option>
                                <option value="zone_id" {{ old('client_field', $serviceOption->client_field) == 'zone_id' ? 'selected' : '' }}>الزون اي دي</option>
                                <option value="email" {{ old('client_field', $serviceOption->client_field) == 'email' ? 'selected' : '' }}>الايميل</option>
                                <option value="server_name" {{ old('client_field', $serviceOption->client_field) == 'server_name' ? 'selected' : '' }}>اسم السرفر</option>
                                <option value="mobile" {{ old('client_field', $serviceOption->client_field) == 'mobile' ? 'selected' : '' }}>رقم الهاتف</option>
                                <option value="link" {{ old('client_field', $serviceOption->client_field) == 'link' ? 'selected' : '' }}>رابط الحساب</option>
                                <option value="address_bsc" {{ old('client_field', $serviceOption->client_field) == 'address_bsc' ? 'selected' : '' }}>معرف BSC</option>
                                <option value="address_trc20" {{ old('client_field', $serviceOption->client_field) == 'address_trc20' ? 'selected' : '' }}>معرف TRC20</option>
                            </select>
                            @error('client_field')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="col-span-1">
                            ...
                            <select id="price_type" 
                                ...
                                <option value="fixed" {{ old('price_type', $serviceOption->price_type) == 'fixed' ? 'selected' : '' }}>{{ __('app.price_fixed') }}</option>
                                <option value="percent" {{ old('price_type', $serviceOption->price_type) == 'percent' ? 'selected' : '' }}>{{ __('app.price_percent') }}</option>
                            </select>
                            @error('price_type')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="col-span-1" id="quantity_div">
                            ...
                            <input type="text" 
                                   id="quantity" 
                                   name="quantity" 
                                   value="{{ old('quantity', $serviceOption->quantity) }}"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                        ...
                        <div class="col-span-1" id="fixed_price_div">
                            ...
                            <input type="number" 
                                   id="price_fixed" 
                                   name="price_fixed" 
                                   value="{{ old('price_fixed', $serviceOption->price_fixed) }}"
                                   step="0.01"
                                   min="0"
                        ...
                        <div class="col-span-1" id="percent_price_div" style="display: none;">
                            ...
                            <input type="number" 
                                   id="price_percent" 
                                   name="price_percent" 
                                   value="{{ old('price_percent', $serviceOption->price_percent) }}"
                                   step="0.01"
                                   min="0"
                                   max="100"
                        ...
                        <div class="col-span-1" id="provider_price_div">
                            ...
                            <input type="number" 
                                   id="provider_price" 
                                   name="provider_price" 
                                   value="{{ old('provider_price', $serviceOption->provider_price) }}"
                                   step="0.001"
                                   min="0.001"
                    ...
                    <div class="mt-6">
                        ...
                        <input type="text" 
                               id="extra_info" 
                               name="extra_info" 
                               value="{{ old('extra_info', $serviceOption->extra_info) }}"
                    ...
                    <div class="mt-6">
                        ...
                        <input type="number" 
                               id="sort_order" 
                               name="sort_order" 
                               value="{{ old('sort_order', $serviceOption->sort_order) }}"
                               min="0"
                        ...
                        <div class="mt-2">
                            <label class="inline-flex items-center">
                                <input type="checkbox" 
                                       name="is_active" 
                                       value="1"
                                       {{ old('is_active', $serviceOption->is_active) ? 'checked' : '' }}
                                       class="form-checkbox">
