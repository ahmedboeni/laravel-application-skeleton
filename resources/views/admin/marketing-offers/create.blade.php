...
@section('title', 'إنشاء عرض تسويقي جديد')
...
<div class="py-6">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900 dark:text-white mb-2">
                        إنشاء عرض تسويقي جديد
                    </h1>
                    <p class="text-gray-600 dark:text-gray-400">
                        إنشاء عرض تسويقي جديد مع الخصومات
                    </p>
                </div>
                <div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('admin.marketing-offers.store') }}" class="space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" 
                                  class="form-textarea" placeholder="أدخل وصف العرض" required>{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="discount_type" id="discount_type" class="form-select" required onchange="toggleDiscountFields()">
                                    ...
                                    <option value="both" {{ old('discount_type') == 'both' ? 'selected' : '' }}>كلاهما</option>
                                </select>
                                @error('discount_type')
                                    <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div id="percentage_field" class="discount-field" style="display: none;">
                                ...
                                <input type="number" name="discount_percentage" id="discount_percentage" 
                                       value="{{ old('discount_percentage') }}" class="form-input" 
                            ...
                            <div id="fixed_field" class="discount-field" style="display: none;">
                                ...
                                <input type="number" name="discount_amount" id="discount_amount" 
                                       value="{{ old('discount_amount') }}" class="form-input" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            <div>
                                ...
                                <input type="number" name="min_purchase_amount" id="min_purchase_amount" 
                                       value="{{ old('min_purchase_amount', 0) }}" class="form-input" 
                            ...
                            <div>
                                ...
                                <input type="number" name="max_discount_amount" id="max_discount_amount" 
                                       value="{{ old('max_discount_amount') }}" class="form-input" 
                                       placeholder="0.00" step="0.01" min="0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="usage_limit" id="usage_limit" 
                                       value="{{ old('usage_limit') }}" class="form-input" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="datetime-local" name="valid_from" id="valid_from" 
                                       value="{{ old('valid_from') }}" class="form-input">
                                @error('valid_from')
                                    <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="valid_until" id="valid_until" 
                                       value="{{ old('valid_until') }}" class="form-input">
                                @error('valid_until')
                                    <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                                @enderror
                            </div>
                                ...
                                <select name="excluded_services[]" id="excluded_services" class="form-select" multiple>
                                    @foreach($services ?? [] as $service)
                                        <option value="{{ $service->id }}" 
                                                {{ in_array($service->id, old('excluded_services', [])) ? 'selected' : '' }}>
                                            {{ $service->name }}
                                        </option>
                                    @endforeach
                                </select>
